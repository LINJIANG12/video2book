...
# ---------------------------------------------------------------------------
# 3) 重复分集复用：只认分集逐字稿，任务书不是复用源
# ---------------------------------------------------------------------------

def _dup_audio_ws(make_workspace, name: str):
    ws = make_workspace(name)
    for filename in ("P01_绪论.m4a", "P02_绪论重复.m4a"):
        (ws.audio_dir / filename).write_bytes(b"x" * 20000)
    return ws


def test_sync_duplicate_assets_reuses_episode_transcript(make_workspace):
    """重复分集之间逐字稿等价：整份拷贝，不必重听一遍音频。"""
    ws = _dup_audio_ws(make_workspace, "去重_复用逐字稿")
    transcript = "这是一份正式逐字稿。" * 100
    (ws.subtitles_dir / "P01_绪论_逐字稿.md").write_text(transcript, encoding="utf-8")
    (ws.subtitles_dir / "P01_绪论_转录任务书.md").write_text("这是任务书。" * 100,
                                                            encoding="utf-8")
...
def test_sync_duplicate_assets_copies_transcript_not_task_file(make_workspace):
    """复用的必须是正式逐字稿本身；同目录同前缀的任务书一旦被误拷就是脏语料。"""
    ws = _dup_audio_ws(make_workspace, "去重_不误拷任务书")
    transcript = "这是一份正式逐字稿。" * 100
    (ws.subtitles_dir / "P01_绪论_逐字稿.md").write_text(transcript, encoding="utf-8")
    (ws.subtitles_dir / "P01_绪论_转录任务书.md").write_text("这是任务书。" * 100,
                                                            encoding="utf-8")
    ws.sync_duplicate_assets()
    dst = ws.subtitles_dir / "P02_绪论重复_逐字稿.md"
    assert dst.read_text(encoding="utf-8") == transcript, "复用源不是正式逐字稿（任务书被误拷）"


def test_sync_duplicate_assets_reports_nothing_without_transcript(make_workspace):
    """没有分集逐字稿时不报同步：块级链路不靠它派发，报了只会让人以为占了便宜。"""
    ws = _dup_audio_ws(make_workspace, "去重_无逐字稿源")
    (ws.subtitles_dir / "P01_绪论_转录任务书.md").write_text("这是任务书。" * 100,
                                                            encoding="utf-8")
    assert ws.sync_duplicate_assets() == [], "无逐字稿源时不应报告同步"


def test_sync_duplicate_assets_reuse_is_idempotent(make_workspace):
    """再跑一遍不得重复上报：目标已存在即视为无需再同步。"""
    ws = _dup_audio_ws(make_workspace, "去重_幂等")
    (ws.subtitles_dir / "P01_绪论_逐字稿.md").write_text("正式逐字稿。" * 50,
                                                        encoding="utf-8")
    assert len(ws.sync_duplicate_assets()) == 1
    assert ws.sync_duplicate_assets() == [], "已同步过的分集被重复同步"
