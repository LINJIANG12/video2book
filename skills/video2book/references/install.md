...
一条硬依赖：**Python 3.10+**。**系统 `ffmpeg`**（在 `PATH`）与**听音通道之一**
（MCP 工具 `read_audio` 或 `read_media`，由配套仓库 [LINJIANG12/omni-media](https://github.com/LINJIANG12/omni-media)
提供，当前契约版本为 1）**只在出现缺字幕块时才需要**：B 站课程字幕完整时，阶段一不下载任何音频，
规划与逐字稿全程用不到它们。但本地课程、YouTube、抖音链路，以及任何缺字幕的块，都必须先把这两样备齐。
**服务装在哪都可以**——技能在用时按宿主自己的工具列表判定通道，不需要配置路径。

> 完整的依赖清单（含 `ffprobe`、Python 3.12、`git` 这些可选降级项）与「缺了会怎样、该怎么办」
> 见 [`runtime.md` §1](runtime.md)——本节只列装之前要确认的项，不重复那张表。
