...
<p>
  <strong>把 B 站、YouTube、抖音长视频与本地课程音视频，字幕优先地按块取出逐字稿，直出精读教材长文，再整编为模块全书与思维导图复习笔记。</strong>
  <br />
  <em>两阶段流水线 · 计划优先 · 字幕优先 · 缺块才听音 · 三轨交付 · 交付前机器门禁 · Python 3.10+ · 多平台统一媒体内核</em>
</p>
...
给出一门课的链接或目录，它先按分集时长把课程切成 40–60 分钟的知识块，字幕优先拿块级逐字稿（只有缺字幕的块才取音转录），**一块成一篇**模块长文，再按块序整编出模块教材与复习笔记。
...
长课程的直接难点是听不完、也记不住。这个技能先由课程元数据（集号与时长）把课程切成 40–60 分钟的知识块（超长集劈上下），块计划落在工作区根目录 `block_plan.json`，**规划阶段不下载任何音频**；块级逐字稿优先取 B 站中文字幕，字幕完整的块一个字节音频都不下，缺字幕的块才按计划物化音频、交给听音通道转录。写作角色读块级逐字稿，一块写一篇模块长文，不再接触音频。按集切分只是事后查阅的可选动作，不在主链上。取音调用次数因此从「每集一次」降到「每块一次」，全课都有字幕时降到零（实测 9 门课 936 集 → 381 块，降 2.46 倍）。

产出分三轨，各自落在独立目录，可以单独取用：模块精读长文、模块合辑教材、跨块复习笔记。每个产物在交付前都要过一遍机器门禁——套话填充、空壳标题、分集平铺标题这类问题会被脚本拦下，而不是留给你在阅读时发现。

块级逐字稿由字幕阶段优先产出，缺字幕块按需听音兜底。只有真的走到听音兜底时才需要准备一个听音通道，它由配套仓库 [omni-media][link-omni-media] 提供：宿主自带音频模态时用它的 `read_audio`（零凭证），只有文本能力时用它的 `read_media`（由外部模型代读）——两者是同一个包的两条通道。字幕完整的课程全程不碰音频，也就完全用不上它；真有缺字幕块又没有听音通道时，流水线会停下提示你挂载。装好之后，一条命令就能跑完一门课。
...
├── subtitles/     BLKxx_*_逐字稿.md           # 块级逐字稿（写作唯一事实源）
├── block_plan.json                          # v4 逻辑块计划（模块边界唯一事实源）
├── audio/         PXX_*.m4a                 # 16kHz 单声道音频切片
├── parts.json                               # 工作区集号拓扑（阶段二取集号基准）
└── manifest.json                            # 账本（可由 sync 按磁盘回填）
```

复习笔记首行直接以 H1 进主题，概念块采用高密度的逐级缩进骨架（下为真实产物节选）：
...
**真实产物示例**：配套仓库 [video2book-courses][link-courses] 归档了跑完整流水线后的全部交付物（包含十余门课程的模块长文、合辑全书、复习笔记与逐字稿，详见该仓发布的最新清单）。想先看输出质量再决定是否安装，可以直接从那里读起。
...
> [!IMPORTANT]
> 第三步的听音通道只在**缺字幕块兜底**时需要：B 站课程字幕完整时，阶段一不下载音频、不调用听音通道。两条通道（`read_audio` / `read_media`）由配套仓库 [omni-media][link-omni-media] 的**同一个包**提供，二选一即可；真有缺字幕块又没有它时，流水线会停下提示你挂载。
...
# 3) 听音通道（仅缺字幕块兜底时必需）：来自配套仓库 omni-media（单一包，两条通道二选一）
...
flowchart TD
    A[多平台摄取<br/>Bilibili · YouTube · 抖音 · 本地] --> B[课程元数据<br/>集号 + 时长 → parts.json]
    B --> M[计划优先装箱<br/>block_plan.json（不碰音频）]
    M --> S{B 站字幕完整？}
    S -->|完整块| T[直接写块级逐字稿<br/>零下载、零听音]
    S -->|缺字幕块| N[只为缺字幕块物化音频<br/>FFmpeg 16kHz 单声道]
    N --> C{课程总时长 ≤ 60 分钟}
    C -->|是| D[主 Agent 串行转录]
    C -->|否| E[派发转录角色<br/>按块并行取音]
    D --> F[块级听音转录<br/>read_audio / read_media]
    E --> F
    T --> W[写作角色读块级逐字稿<br/>一块一篇模块长文]
    F --> W
...
- **取音只发生在转录角色身上，且只对缺字幕块按块取**：逻辑块先由元数据写入根目录 `block_plan.json`；字幕完整的块直接出逐字稿、不下载音频，缺字幕块才按既有计划物化音频，绝不重新规划。写作角色只读块级逐字稿，一个块写一篇模块长文，不接触音频。
- **派发阈值写在 `src/core/budget.py`**：课程总时长在 60 分钟以内时由主 Agent 串行处理，超过则必须派发——转录角色建议 3 个并行消费块队列，写作角色**一个块一个子智能体**（一块一篇模块长文）。窗口兜底只对**走通道 A 的转录角色**成立：实算音频 token 超过上下文窗口 60% 的块必须分卷续读。
- **模块层没有规划，只有归并一趟，且缺归并不停机**：块就是知识模块（按分集时长装箱成 40–60 分钟的块，块标题由块内分集名语义组合而来），教材直接按块序整编块长文；笔记侧把块归并成若干篇（`note_plan.json`，一篇可跨多个块）。漏认领／重复认领／引用不存在的块都当场抢救，命令始终正常退出，盘上的 `note_plan.json` 不会被兜底结果覆盖。
...
- **外部程序** — `ffmpeg`，需加入 `PATH`，是缺字幕块取音频/切片的硬前置（字幕完整的课程不下载音频，也用不到它）；`ffprobe` 可选（缺失时改用 `ffmpeg -i` 解析时长）
- **听音通道** — `read_audio` 或 `read_media`，二者其一，由配套仓库 [omni-media][link-omni-media] 提供；同样只在缺字幕块兜底时需要
...
**9 个子命令的完整清单、每个开关的用途、全部参数、逐场景示例与退出码（0–4）** 集中在
...
不能。字幕完整的 B 站课程可以由 `pipeline` 直接产出逐字稿；缺字幕块仍需要听音通道。两条听音通道 `read_audio` 与 `read_media` 都由 [omni-media][link-omni-media] 提供，缺字幕块无法物化音频时流水线会停下提示你挂载其一。

### 该挂哪个入口

按「能直写落盘者优先」排序。优先挂 `omni-media-ext` 入口（固定 `--mode ext`，调用 `read_media` 支持 `output_file` 直写落盘、0 Token 进上下文，防摘要冒充逐字稿）；工具列表里没有 `read_media` 或宿主具备原生音频模态且偏好零凭证时，再挂 `omni-media` 入口（固定 `--mode native`，使用 `read_audio` 取切片聆听）。**两者是同一个包的两条通道**，装一次即可，切换只需换注册名。两条通道分页契约同构。
